/**
 * Layout.tsx  (updated — Analytics nav link added)
 * --------------------------------------------------
 * Replace:  web-dashboard/src/components/Layout.tsx
 *
 * Changes vs original:
 *   1. Import BarChart2 from lucide-react
 *   2. Add Analytics entry to navLinks (between History and Admin group)
...
// ─── Nav links ──────────────────────────────────────────────────────────────
// Only change vs original: added the "Analytics" entry (★)
...
const navLinks: NavLink[] = [
  ...
  { label: 'Analytics',          path: '/analytics',          icon: <BarChart2   size={17} /> }, // ★ NEW
...
export const Layout: React.FC<LayoutProps> = ({
  ...
  useEffect(() => {
    ...
    if (!currentUser && storedEmail) {
      setCurrentUser({ displayName: storedEmail.split('@')[0], email: storedEmail, photoURL: null });
    }
  }, [currentUser]);
  ...
  const handleLogout = async () => {
    try {
      await signOut();
      ['auth_token', 'user_id', 'user_email', 'user_role'].forEach(k =>
      ...
      >
        ...
        <nav className="flex-1 px-3 space-y-0.5 overflow-y-auto pb-4">
          {navLinks.map((link, index) => {
            ...
            const showGroup    = link.group && (!prevLink || prevLink.group !== link.group);
            ...
            return (
              <div key={link.path}>
                ...
                <Link
                  ...
                  className={`nav-item flex items-center rounded-xl px-3 py-2.5 ${
        ...
        <div className="px-3 pb-5 pt-2">
          ...
          <button
            ...
            className={`nav-item w-full flex items-center rounded-xl px-3 py-2.5 btn-press ${
              !sidebarOpen ? 'justify-center' : 'gap-3'
            }`}
                ...
                {dropdownOpen && (
                  ...
                  >
                    ...
                    {[
                      ...
                      { to: '/analytics', icon: <BarChart2 size={14} />, label: 'Analytics'  }, // ★ shortcut
