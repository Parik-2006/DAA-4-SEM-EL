"""
admin.py  –  Enhanced admin API router  (SECURITY-HARDENED)
============================================================
All endpoints require the "admin" JWT role via the require_admin dependency.
Write operations (POST/PUT/DELETE) are additionally logged to audit_logs.

Endpoints  (unchanged signatures — security is additive)
---------
Existing (preserved):
...
# ===========================================================================
# EXISTING ENDPOINTS  (preserved — admin dep added)
# ===========================================================================

@router.get("/students", response_model=List[dict])
async def get_all_students(
    admin: UserContext = require_admin,          # ← SECURITY
):
    ...
    try:
        ...
        if not fb.fs:
            return {"totalStudents": 0, "presentToday": 0, "absentToday": 0,
                    "lateToday": 0, "attendanceRate": 0.0, "error": "Firestore not initialized"}
        ...
        present_ids = [r.get("student_id") for r in today_records
    ...
    try:
        ...
        for doc in docs:
            ...
            records.append({
                ...
                "studentName": v.get("studentName") or v.get("metadata", {}).get("student_name", "Unknown"),
        ...
        for r in records:
            if not r["studentName"] or r["studentName"] == "Unknown":
                stu_doc = fb.fs.collection("students").document(r["studentId"]).get()
                if stu_doc.exists:
                    r["studentName"] = stu_doc.to_dict().get("name", "Unknown")

        records.sort(key=lambda x: x.get("markedAt", ""), reverse=True)
        start = (page - 1) * limit
        return {"data": records[start:start + limit], "total": len(records),
...
async def create_cie(
    ...
    get_audit_service().log(
        action="CREATE_CIE", resource="cie", resource_id=cie_id,
        user=admin, request=request, after=record,
    )
...
async def list_cies(
    ...
    fb  = _fb()
    raw = fb.get_reference("cies").get() or {}
    ...
    start = (page - 1) * limit
    return {"data": cies[start:start + limit], "total": len(cies), "page": page, "limit": limit,
            "total_pages": max(1, (len(cies) + limit - 1) // limit)}


@router.put("/cie/{cie_id}")
async def update_cie(
    cie_id: str,
    payload: CIEUpdateSchema,
    request: Request,
    admin: UserContext = require_admin,          # ← SECURITY
):
    fb  = _fb()
    ...
    if payload.academic_year is not None: updates["academic_year"] = payload.academic_year.strip()
    if payload.department  is not None: updates["department"]    = payload.department

    ref.update(updates)

    get_audit_service().log(
        action="UPDATE_CIE", resource="cie", resource_id=cie_id,
        user=admin, request=request, before=existing, after={**existing, **updates},
    )
...
async def delete_cie(
    ...
    get_audit_service().log(
        action="DELETE_CIE", resource="cie", resource_id=cie_id,
        user=admin, request=request, before=cie,
        details={"cascaded_class_deletions": len(class_ids)},
    )
    return {"success": True, "cie_id": cie_id, "deleted_classes": len(class_ids),
...
async def create_class(
    ...
    existing_ids = cie.get("class_ids", [])
    existing_ids.append(class_id)
    cie_ref.update({"class_ids": existing_ids, "updated_at": now})

    get_audit_service().log(
        action="CREATE_CLASS", resource="class", resource_id=class_id,
        user=admin, request=request, after=record,
    )
...
async def list_classes(
    ...
    fb  = _fb()
    raw = fb.get_reference("classes").get() or {}
    ...
    start = (page - 1) * limit
    return {"data": classes[start:start + limit], "total": len(classes), "page": page, "limit": limit,
...
async def update_class(
    ...
    get_audit_service().log(
        action="UPDATE_CLASS", resource="class", resource_id=class_id,
        user=admin, request=request, before=existing, after={**existing, **updates},
    )
...
async def bulk_import_timetable(
    ...
    if not parse_result.rows and not parse_result.ok:
        return {"success": False, "total_rows": parse_result.total_rows, "imported": 0,
                "failed": parse_result.total_rows,
                "parse_errors": [e.to_dict() for e in parse_result.errors], "conflict_errors": []}
    ...
    get_audit_service().log(
        action="UPLOAD_TIMETABLE", resource="timetable", user=admin, request=request,
        details={"imported": len(imported_ids), "failed": total_failed,
                 "filename": file.filename},
    )
    return {
        "success": len(conflicts) == 0 and len(parse_result.errors) == 0,
        "total_rows": parse_result.total_rows,
        "imported": len(imported_ids), "failed": total_failed,
        "parse_errors": [e.to_dict() for e in parse_result.errors],
        "conflict_errors": [{"rows": [c["period_a"]["row"], c["period_b"]["row"]],
                             "message": c["message"]} for c in conflicts],
...
async def bulk_import_students(
    ...
    for row in validation["valid_rows"]:
        try:
            ...
            fb.get_reference(f"users/{row['student_id']}").set({
                "id": row["student_id"], "name": row["name"], "email": row["email"],
                "role": "student", "class_id": row["class_id"],
    ...
    get_audit_service().log(
        action="BULK_IMPORT_STUDENTS", resource="student", user=admin, request=request,
        details={"imported": len(imported), "failed": total_failed, "filename": file.filename},
    )
    return {
        "success": parse_result.ok and validation["valid"] and len(persist_errors) == 0,
        "total_rows": parse_result.total_rows, "imported": len(imported), "failed": total_failed,
        "parse_errors": [e.to_dict() for e in parse_result.errors],
        "validation_errors": [{"row": r["row"], "student_id": r.get("student_id"), "errors": r["errors"]}
...
async def set_system_config(
    ...
    get_audit_service().log(
        action="CONFIG_CHANGE", resource="system_config", user=admin, request=request,
        after=updates, details={"changed_fields": [k for k in updates if k != "updated_at"]},
    )
    return {"success": True, "updated_fields": [k for k in updates if k != "updated_at"],
            "config": updates}